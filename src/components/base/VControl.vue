...
const hasError = computed(() =>
  field?.value ? Boolean(field?.value?.errorMessage?.value || (field?.value?.errorMessage != undefined)) : props.hasError
)
...
<template>
  <div :class="controlClasees">
    ...
    <a v-else-if="hasError" class="validation-icon is-error" role="button" tabindex="0"
      @click.prevent="() => field?.resetField?.()" @keydown.enter.prevent="() => field?.resetField?.()">
      <VIcon icon="lucide:x" />
    </a>
