...
/**
 * Gaiso Materials Data
 */
const isFetchingReport = ref(false);
const gaisoMaterials = ref<GaisoMaterials[]>([]);
const page = ref(1)
const limit = ref(10)
const total = ref(0)
...
const gaisoMaterialsData = async () => {
  ...
  await useApi.get('auxiliary-material', {
    ...
    // Map API response to GaisoMaterials type
    const mappedData = res.data.map((item: any): GaisoMaterials => ({
      akhir: item.akhir || 0,
      awal: item.awal || 0,
    ...
    gaisoMaterials.value = mappedData;
...
<template>
  <div class="sales-dashboard">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h3 class="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
              Laporan Pertanggung Jawaban Mutasi Bahan Penolong (Gaiso)
            </h3>
            ...
            <div class="flex flex-wrap gap-4 items-end">
                  ...
                  <div class="booking-bar-inputs">
                    <VControl class="control mr-4">
                      <DatePicker v-model="filterRange.start" dateFormat="dd-mm-yy"
                        inputClass="datepicker-input w-full px-[38px] py-3 bg-white border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-700 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 transition-all duration-200 hover:border-gray-300 appearance-none cursor-pointer shadow-sm"
                ...
                <div class="relative">
                  <input type="text" v-model="itemCode" @input="handleFilterChange" placeholder="Masukkan Kode Item"
                    class="w-full px-3 py-2 pr-10 bg-white border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-700 
                           focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 transition-all duration-200 
              ...
              <div class="filter-group ml-auto">
                <button @click="applyFilter"
                  class="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 
                         text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 
                         transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-emerald-200 flex items-center justify-center space-x-2">
    ...
    <GaisoMaterialsTable :items="gaisoMaterials" :isFetching="isFetchingReport" :page="page" :limit="limit"
      ...
      @change-limit="handleChangeLimit" :lap="'Gaiso'" />
  </div>
</template>
