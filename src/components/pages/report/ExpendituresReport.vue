...
const expenditureProducts = ref<ExpenditureProduct[]>([]);
const page = ref(1)
const limit = ref(10)
const total = ref(0)
...
const expenditureProductsData = async () => {
  isFetchingReport.value = true;
  await useApi.get('report/expenditure-products', {
    page: page.value,
    limit: limit.value,
    ...
    // Map API response to ExpenditureProduct type
    const mappedData = res.data.map((item: any, idx: number): ExpenditureProduct => ({
      no: ((page.value - 1) * limit.value) + idx + 1,
      jenis_pabean: item.jenis_pabean || '',
      no_pabean: item.no_pabean || '',
      tgl_pabean: item.tgl_pabean || '',
      no_bukti: item.trans_no || '',
      tgl_bukti: item.trans_date || '',
      penerima_barang: item.cust_name || '',
      kode_barang: item.item_code || '',
      nama_barang: item.item_name || '',
      jumlah: Number(item.dlv_qty) || 0,
      sat: item.sales_unit || '',
    ...
    expenditureProducts.value = mappedData;
