...
// Filter variables
const startDate = ref(route.query.start_date ? dayjs(route.query.start_date as string || new Date()).toDate() : dayjs().startOf('month').toDate())
  ...
  () => {
    debounceTimer = setTimeout(() => {
      router.replace({
        ...
        query: {
          start_date: startDate.value ? dayjs(startDate.value).format('YYYY-MM-DD') : null,
        }
      });
      handleFilterChange();
    }, 500);
...
const wipPosition = ref<WIPPosition[]>([]);
const page = ref(1)
const limit = ref(10)
const total = ref(0)
...
const wipPositionData = async () => {
  isFetchingReport.value = true;
  await useApi.get('report/wip-position', {
    page: page.value,
    limit: limit.value,
    from: dayjs(startDate.value).format('YYYY-MM-DD'),
    item_code: productCode.value || '',
    item_name: productName.value || '',
  }, false).then((res: any) => {
    // Map API response to WIPPosition type
    const mappedData = res.data.map((item: any, idx: number): WIPPosition => ({
    ...
    <div
      ...
      <div class="flex flex-col lg:justify-between gap-6 w-full">
        ...
        <div class="flex items-center space-x-4">
          <div class="p-3 bg-gradient-to-br from-red-500 to-orange-600 rounded-2xl shadow-lg">
            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M16 11V7a4 4 0 00-8 0v4M8 11v6a2 2 0 002 2h4a2 2 0 002-2v-6m-6 0h6">
              </path>
            </svg>
          </div>
          <div>
