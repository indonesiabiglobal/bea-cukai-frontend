...
const props = withDefaults(defineProps<{ items: TopGuarantorIncome[]; isFetching: boolean; limit?: number }>(), {
  limit: 10,
})
...
const options = computed(() => ({
  ...
  plotOptions: {
    bar: {
      ...
      borderRadius: 6,
      barHeight: '60%',
      dataLabels: { position: 'center' }, // label berada di tengah bar
    },
  },
  ...
  dataLabels: {
    ...
    // PILLS agar tetap kebaca walau bar kecil
    style: {
      fontSize: '11px',
      fontWeight: 700,
      colors: ['#ffffff'],
  ...
  <div class="w-full mb-4">
    <div class="bg-white rounded-lg shadow p-3">
      ...
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-sm font-semibold text-gray-700">Top Guarantors</h3>
        <div class="inline-flex rounded-lg overflow-hidden border border-gray-200">
          <button class="px-3 py-1 text-xs" :class="metric==='net' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'" @click="metric='net'">Net</button>
          <button class="px-3 py-1 text-xs" :class="metric==='debit' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'" @click="metric='debit'">Debit</button>
          <button class="px-3 py-1 text-xs" :class="metric==='credit' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'" @click="metric='credit'">Credit</button>
        </div>
      </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-100 bg-white">
                <tr v-for="(r, idx) in topN" :key="idx" class="text-sm">
                  <td class="px-3 py-2">
                    <div class="font-medium text-gray-900 truncate max-w-[100px]" :title="r.name">{{ r.name || r.code }}</div>
                  ...
                  <td class="px-3 py-2 text-right bg-gray-50" :class="r.net >= 0 ? 'text-emerald-600' : 'text-rose-600'">{{ cf.format(r.net) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
