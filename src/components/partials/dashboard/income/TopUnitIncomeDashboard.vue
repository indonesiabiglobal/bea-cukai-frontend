...
const props = withDefaults(defineProps<{ items: TopUnitIncome[]; isFetching: boolean; limit?: number }>(), {
  limit: 10,
})
...
const options = computed(() => ({
  ...
  title: { text: 'Top Units', align: 'center' },
  plotOptions: {
    bar: {
      horizontal: true,
      borderRadius: 6,
  ...
  dataLabels: {
    ...
    style: {
      fontSize: '11px',
      fontWeight: 700,
      colors: ['#ffffff'], // gray-900
    },
    background: {
      enabled: true,
      foreColor: themeColors.primary,    // warna teks
      padding: 3,
      borderRadius: 4,
      borderWidth: 0,
  ...
  <div class="w-full mb-4">
    <div class="bg-white rounded-lg shadow p-3">
      ...
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-sm font-semibold text-gray-700">Top Units</h3>
          ...
          <div class="overflow-hidden rounded-xl border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-100 bg-white">
                <tr v-for="(unit, idx) in topN" :key="idx" class="text-sm">
                  <td class="px-3 py-2">
