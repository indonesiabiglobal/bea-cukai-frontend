...
const props = defineProps<{ summary: PurchaseSummary | null | undefined; isFetching: boolean }>()
...
const uniqueVendors = computed(() => toNum(props.summary?.unique_vendors))
const uniqueItems = computed(() => toNum(props.summary?.unique_items))
const txCount = computed(() => toNum(props.summary?.tx_count))
const avgUnitCost = computed(() => (totalQty.value > 0 ? Math.round(totalSubtotal.value / totalQty.value) : 0))
</script>
  ...
  <div class="w-full mb-4">
    <div class="bg-white rounded-2xl shadow p-4 md:p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-base md:text-lg font-semibold text-gray-800">Purchase Summary</h3>
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <div class="p-4 rounded-xl bg-gradient-to-br from-blue-50 to-white border border-blue-100">
          <div class="text-xs font-medium text-blue-700">Total Spend</div>
        ...
        <div class="p-4 rounded-xl bg-gradient-to-br from-amber-50 to-white border border-amber-100">
          <div class="text-xs font-medium text-amber-700">Avg Unit Cost</div>
          <div class="mt-1 text-xl md:text-2xl font-bold text-amber-900">{{ cf.format(avgUnitCost) }}</div>
        </div>
        
        <!-- Unique Vendors -->
        <div class="p-4 rounded-xl bg-gradient-to-br from-fuchsia-50 to-white border border-fuchsia-100">
          <div class="text-xs font-medium text-fuchsia-700">Unique Vendors</div>
          <div class="mt-1 text-xl md:text-2xl font-bold text-fuchsia-900">{{ nf.format(uniqueVendors) }}</div>
        </div>
