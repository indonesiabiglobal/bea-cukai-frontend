...
const props = withDefaults(defineProps<{ items: SaleByCategory[]; isFetching: boolean; limit?: number }>(), {
  limit: 20,
})
...
// ===== Export Excel (Top & All) =====
function fileName() {
  const d = new Date()
  const ymd = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
  return `Sales_By_Category_${ymd}.xlsx`
}
function exportSaleCategoryExcel() {
  ...
  const header = ['No', 'Category Code', 'Category Name', 'Qty', 'Subtotal (IDR)', 'Share by Subtotal %', 'Share by Qty %']
  ...
  aoa.push(['', `TOTAL (${`All ${data.length}`})`, '', Number(totalQty), Number(totalSubtotal), 100, 100])

  const ws = XLSX.utils.aoa_to_sheet(aoa)
  ws['!cols'] = [{ wch: 4 }, { wch: 16 }, { wch: 28 }, { wch: 12 }, { wch: 18 }, { wch: 18 }, { wch: 16 }]

  const lastRow = aoa.length - 1
  ws['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: lastRow, c: header.length - 1 } }) }

  const fmtInt = '#,##0', fmtMoney = '#,##0', fmtPct = '0.0%'
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="inline-flex items-center gap-2">
          ...
          <button
            ...
            :disabled="isFetching || !items?.length" @click="exportSaleCategoryExcel()">
            Export
          </button>
        </div>
      </div>
