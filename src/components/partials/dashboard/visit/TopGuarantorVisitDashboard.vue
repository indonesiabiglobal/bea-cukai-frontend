...
const props = withDefaults(defineProps<{ items: TopGuarantorVisit[]; isFetching: boolean; limit?: number }>(), {
  limit: 10,
})

// theme fallback (pakai composable-mu kalau ada)
...
const rows = computed(() =>
  (props.items ?? [])
    .map(r => ({ name: r.name?.trim() || 'UNKNOWN', visits: toNum(r.visits) }))
    .sort((a, b) => b.visits - a.visits)
)

const topGuarantors = computed(() => rows.value.slice(0, props.limit))

// Data untuk chart
const categories = computed(() => topGuarantors.value.map(r => r.name))
const series = computed(() => [{ name: 'Visits', data: topGuarantors.value.map(r => r.visits) }])
...
const yBounds = computed(() => {
  const max = Math.max(0, ...topGuarantors.value.map(r => r.visits))
  return { min: 0, max: niceMax(max), ticks: 4 }
})

// Opsi chart: label di kanan bar (selalu kebaca)
const options = computed(() => ({
  chart: { type: 'bar', toolbar: { show: false } },
  title: { text: 'Top Guarantors', align: 'center' },
  plotOptions: {
    bar: {
      horizontal: true,
      borderRadius: 6,
...
function fileName() {
  ...
  return `Visits_Guarantors_${ymd}.xlsx`
}

function exportTopGuarantorsExcel() {
  const data = rows.value
  if (!data.length) return

  const totalVisits = data.reduce((s, r) => s + toNum(r.visits), 0)

  const header = ['No', 'Guarantor', 'Visits', 'Share %']
  const aoa: (string | number)[][] = [header]

  data.forEach((r, i) => {
    const share = totalVisits > 0 ? (toNum(r.visits) / totalVisits) : 0
    aoa.push([i + 1, r.name, Number(r.visits), Number(share)])
  })
  ...
  <div class="w-full mb-4">
    <div class="bg-white rounded-2xl shadow p-4 md:p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-base md:text-lg font-semibold text-gray-800">Top Guarantors</h3>
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            :disabled="isFetching || !items?.length" @click="exportTopGuarantorsExcel()">
            Export
          </button>
        </div>
      </div>
          ...
          <div class="overflow-hidden rounded-xl border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-100 bg-white">
                <tr v-for="(r, idx) in topGuarantors" :key="idx" class="text-sm">
                  <td class="px-3 py-2">
