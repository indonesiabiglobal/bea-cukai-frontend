...
const handleLogin = async () => {
  if (!isLoading.value) {
    ...
    await useApi.post('/auth/patients/login', data, false).then(async (response) => {
    ...
    if (token.value && !userSession.user) {
      const user = await useApi.get('/profile/patients/user', undefined, false)
            ...
            <form method="post" novalidate :class="[step !== 'login' && 'is-hidden']" class="login-wrapper"
              ...
              <!-- Register -->
                <span>
                  ...
                  <RouterLink to="/auth/signup-patients">Create</RouterLink>
                </span>
              </div>
