...
export function createRouter() {
  return createClientRouter({
     ...
     */
    extendRoutes: (routes) => {
      // const adminRoute = routes.find((r) => r.name === '/admins');

      // if (adminRoute) {
      //   adminRoute.meta ??= {};
      //   adminRoute.meta.requiresAuth = true;
      // } else {
      //   console.warn("Route not found!"); // Log jika tidak ditemukan
      // }

      // const root = routes.find((r) => r.path === '/')
      // if (root) {
      //   root.redirect = '/dashboard/patient'
      //   delete (root as any).component
      //   delete (root as any).components
      // }
