...
export const useLayoutSwitcher = defineStore('layoutSwitcher', () => {
  ...
  const isNavbarRoute = computed(() => route?.fullPath?.startsWith?.('/navbar/') || route?.fullPath?.startsWith?.('/modules/'))
  ...
  const NavbarLayout = defineAsyncComponent({
    ...
    loader: () => import('/@src/layouts/main-navbar.vue'),
    delay: 0,
    suspensible: false,
  })
  ...
  const SidebarLayout = defineAsyncComponent({
    ...
    loader: () => import('/@src/layouts/main-sidebar.vue'),
    delay: 0,
    suspensible: false,
  })
  ...
  function setContentSize(size: any) {
